...
# # Synchronize with Google Sheets 연동
# import gspread
...
# def _get_worksheet():
#     """
#     Streamlit secrets에서 Google 서비스 계정 정보를 읽어
#     assess_sessions 시트를 반환합니다.
#     """
#     creds_dict = dict(st.secrets["gcp_service_account"])
#     creds = Credentials.from_service_account_info(creds_dict, scopes=SCOPES)
#     client = gspread.authorize(creds)

#     # secrets.toml에 설정한 스프레드시트 이름으로 열기
#     spreadsheet = client.open(st.secrets["google_sheets"]["spreadsheet_name"])

#     # 'assess_sessions' 시트가 없으면 새로 만들기
#     try:
#         worksheet = spreadsheet.worksheet("assess_sessions")
#     except gspread.exceptions.WorksheetNotFound:
#         worksheet = spreadsheet.add_worksheet(
#             title="assess_sessions", rows=2000, cols=20
#         )
#         # 헤더 추가
#         worksheet.append_row(CSV_FIELDS, value_input_option="RAW")

#     return worksheet


# # 로컬 CSV fallback (로컬 개발용)
# LOG_DIR = Path(__file__).resolve().parents[1] / "logs"
# ASSESS_CSV = LOG_DIR / "assess_sessions.csv"
...
#     """Streamlit secrets에 gcp_service_account가 있으면 Google Sheets 사용."""
#     try:
...
# def append_assessment_row(row: Dict):
#     """
#     Policy: always append (history-preserving).
#     Google Sheets가 설정돼 있으면 Sheets에, 아니면 로컬 CSV에 저장.
#     """
#     safe_row = {k: row.get(k, "") for k in CSV_FIELDS}
#     if not safe_row["timestamp_utc"]:
#         safe_row["timestamp_utc"] = _now_utc_iso()
...
#         ws = _get_worksheet()
#         # 헤더 순서대로 값 리스트 만들기
#         values = [safe_row[k] for k in CSV_FIELDS]
...
#     if _use_sheets():
#         ws = _get_worksheet()
#         records = ws.get_all_records()  # 헤더 행 제외하고 dict 리스트 반환
